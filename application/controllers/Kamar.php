...
class Kamar extends CI_Controller {
    ...
    function get_kamar_by_indekos(){
      $indekos_id= $this->input->post('indekos_id');

       $this->db->where('id_indekos', $indekos_id);
       $dataarr=  $this->db->get('kamar')->result_array();
...
function get_by_id_kamar(){
      $id_kamar= $this->input->post('id_kamar');

       $this->db->where('id_kamar', $id_kamar);
       $dataarr=  $this->db->get('orders')->result_array();
  ...
  function get_kamar_order_status(){
  ...
  function create_kamar(){
        ...
        /*$this->form_validation->set_rules('id_kamar', 'Kamar Id', 'required');*/
        $this->form_validation->set_rules('id_indekos', 'id indekos', 'required');
        $this->form_validation->set_rules('no_kamar', 'kamar number', 'required');
        $this->form_validation->set_rules('lantai_ke', 'lantai_ke', 'required');


$this->form_validation->set_rules('ukuran', 'id ukuran', 'required');
        $this->form_validation->set_rules('harga', 'harga', 'required');
        $this->form_validation->set_rules('fasilitas', 'fasilitas', 'required');

        $this->form_validation->set_rules('kwh', 'kwh', 'required');
    ...
    $hfgkjf = $this->db->insert('kamar',$insertdata);  
    if($hfgkjf){

      $insertdata['id']  = $hfgkjf ;
   $insertdata['result']  = 'success' ;

   $arasr = array();
   $arasr['data'] = $insertdata;
   $arasr['status'] = '200';

    echo  json_encode($arasr);
